...
%     fprintf('Running SIFT-Feature extraction to training image - no: %d \n',i);
    ...
    % Gather all SIFT descriptors for dimensionality reduction
%     train_SIFT_descriptors{i,1} = lle(double(train_SIFT_descriptors{i,1}),K,d,1,'tmp.txt');
    all_descr_for_reduction = [all_descr_for_reduction double(train_SIFT_descriptors{i,1})];
end

%% LLE dimensionality reduction
% Normalize data
mu = mean(all_descr_for_reduction,1);
X_norm = bsxfun(@minus, all_descr_for_reduction, mu);
...
% Run LLE algorithm for dimensionality reduction
all_descr_for_reduction = lle(X_norm,K,d,1,'tmp.txt');
...
%     fprintf('Running SIFT-Feature extraction to testing image - no: %d \n',i);
   ...
   tmp_image = imread(testingImagesFiles{i,1});
    ...
    %% LLE dimensionality reduction
    test_SIFT_descriptors{i,1} = lle(double(test_SIFT_descriptors{i,1}),8,d,1,'tmp.txt');
end
