...
folder_prefix = 'MNIST_Experiments_60000/';
mkdir(folder_prefix);
...
% Num of neighbors
K = [6; 7; 8; 9; 10; 12; 16; 20; 24; 32; 64];
% K = [8];

% Num of new dimensionality 
d = [10; 16; 20; 24; 32; 40; 52; 64; 96; 128; 256];
% d = [32; 48];
...
for i=1:length(K)
    ...
    for k=1:length(batch_size)
        ...
        tic;
	err = 0;
        err = Classification_with_DimRed(Y, s2, s2Test, N_train, N_test, batch_size(k,1), fid);
	results_classification_err(i,k,length(d)) = err;
        ...
        for j=1:(length(d)-1)
           ...
           %  mkdir(batch_folder_name);
            ...
            % Classification results
            tic;
            err = Classification_with_DimRed(lowerY, s2, s2Test, N_train, N_test, batch_size(k,1), fid);
