...
function err = Classification_without_DimRed(train_SIFT_descr,test_SIFT_descr, train_labels, test_labels, N_train, N_test, fid)
...
%     fprintf('Running classification for test-logo: %d \n',i);
    ...
    %         [matches, scores] = vl_ubcmatch(tmp_parfor_test_SIFT_descriptors,train_SIFT_descriptors{j,1},0.8) ;
            if(~isempty(train_SIFT_descr{j,1}))
                [matches, scores] = vl_ubcmatch(train_SIFT_descr{j,1},tmp_parfor_test_SIFT_descriptors,2.8) ;
