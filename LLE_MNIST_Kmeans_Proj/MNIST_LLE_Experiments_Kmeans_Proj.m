...
folder_prefix = 'MNIST_Experiments_Kmeans_Proj/';
mkdir(folder_prefix);
...
% tmp_labels = gpu_knn(X, c_coord', 2, 1);
...
for i=1:length(K)
    ...
    Y = digits(X, K(i,1), d(end,1), fid, batch_folder_name);

    % Classification results
    tic;
    err = 0;
    err = Classification_with_DimRed_Proj(Y, X, testX, s2, s2Test, Ntrain, N_test, fid);
    ...
    for j=1:(length(d)-1)
        ...
        % Classification results
        tic;
        err = Classification_with_DimRed_Proj(lowerY, X, testX, s2, s2Test, Ntrain, N_test, fid);
